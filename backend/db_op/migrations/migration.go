...
func main() {
	...
	if err := db.AutoMigrate(&models.Questions{}, &models.Users{}, &models.Answers{}, &models.Roles{}, &models.UsersRoles{}); err != nil {
	...
	// 部分インデックスの作成、questionsテーブルのマイグレーション時に使用 (GROMで記述できないため)
	tx := db.Exec(`
	    DO $$
	    BEGIN
